...
namespace XRDevcomfy.PropertyReference.Editor
{
    ...
    public class MethodReferencePropertyDrawer : PropertyDrawer
    {
	private const string kTargetProperty = "Target";
	private const string kMethodName = "MethodName";
	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
	    ...
	    var methodInfoSerialized = property.FindPropertyRelative(kMethodName);
	    string propName = methodInfoSerialized?.stringValue as string;
	...
	GenericMenu BuildPopupList(GameObject target, SerializedProperty property)
	{
	    ...
	    var methodName = property.FindPropertyRelative(kMethodName);
	    ...
	    foreach (var component in target.GetComponents<Component>())
	    {
			...
			{
				menu.AddItem(new GUIContent($"{_type}/{method.Name}"),
							 method.Name == methodName.stringValue && component == targetComponent.objectReferenceValue,
							 () =>
							 {
